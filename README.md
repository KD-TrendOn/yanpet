...
Вы можете использовать `curl` или любой HTTP-клиент для отправки вопроса:
...
Система обрабатывает вопрос асинхронно. Изначально вы получите:
...
Чтобы получить ответ, используйте endpoint `/answer/{question_id}`:

```bash
curl http://localhost:8080/answer/1
```
